...
function getWeightedFee(employee: Employee) {
...
function countRiskFlags(employees: Employee[], flag: RiskFlag) {
  return employees.filter((employee) => employee.riskFlags.includes(flag)).length;
}

export function buildCompanyDiagnosis(employees: Employee[]) {
  const averageReturnRate = average(employees.map((employee) => employee.returnRate));
  const averageFeeRate = average(employees.map((employee) => getWeightedFee(employee)));
  ...
  return {
    summaryCards: [
      ...
      },
      {
        ...
        helper: `분석 직원 ${employees.length}명 기준`
      },
      {
        label: "저성과 직원 비중",
        value: `${employees.length === 0 ? 0 : ((countRiskFlags(employees, "low_return") / employees.length) * 100).toFixed(0)}%`,
        helper: `수익률 부진 ${countRiskFlags(employees, "low_return")}명`
      }
    ],
    benchmarkRows: [
      {
    ...
    riskBreakdown: [
      {
        ...
        label: "위험자산 초과",
        count: countRiskFlags(employees, "risk_asset_over"),
        tone: "bg-red-500"
      },
      {
        flag: "low_return" as const,
        label: "저수익",
        count: countRiskFlags(employees, "low_return"),
        tone: "bg-red-500"
      },
      {
        flag: "high_fee" as const,
        label: "수수료 과다",
        count: countRiskFlags(employees, "high_fee"),
        tone: "bg-amber-500"
      },
      {
        flag: "age_mismatch" as const,
        label: "연령 부적합",
        count: countRiskFlags(employees, "age_mismatch"),
        tone: "bg-amber-500"
      },
      {
        flag: "no_diversification" as const,
        label: "미분산",
        count: countRiskFlags(employees, "no_diversification"),
        tone: "bg-amber-500"
      }
    ],
    ...
    insightLines: [
      averageReturnRate >= benchmarkData.averageReturnRate
        ? `직원 평균 수익률은 ${averageReturnRate.toFixed(1)}%로 업종 평균을 상회합니다.`
        : `직원 평균 수익률은 ${averageReturnRate.toFixed(1)}%로 업종 평균을 하회합니다.`,
      averageFeeRate > benchmarkData.averageFeeRate
        ? `평균 수수료가 업종 평균보다 높아 상품 정리와 전환 제안이 필요한 상태입니다.`
        : `평균 수수료는 업종 평균 범위 내로 관리되고 있습니다.`,
      `위험 플래그는 ${countRiskFlags(employees, "low_return") + countRiskFlags(employees, "high_fee") + countRiskFlags(employees, "risk_asset_over")}건이 핵심 점검 포인트입니다.`
    ]
  };
}
